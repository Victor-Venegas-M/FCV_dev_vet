...
def login(request):
    if request.method == 'POST':
        if request.POST.get('usuario') and request.POST.get('user_pass'):
            ...
            user_data = custom_fx.getUserLoginData(user, upass)

            print(user_data)
            if user_data['id_estado_key'] == 1:
                return redirect(check,user_data['id_estado_key'])
    
    else:    
        return render(request, "login.html")

def check(request, id_user):
    if id_user == 1:
...
def pacientes_detalle(request, id_paciente):
    data = { 'paciente':custom_fx.getPacienteById(id_paciente) }
...
@csrf_exempt
def atenciones_agregar(request):
    if request.method == 'POST':
...
@csrf_exempt
def veterinarios_agregar(request):
    if request.method == 'POST':
